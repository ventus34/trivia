...
6. **Progressive Web App (PWA) & Offline-First**:
   ...
   - Runtime cache with a _Stale-While-Revalidate_ strategy for dynamic categories, enabling offline gameplay for previously played categories while automatically updating when online.
...
6. **Aplikacja PWA i Tryb Offline (Offline-First)**:
   ...
   - Strategia _Stale-While-Revalidate_ dla kategorii wczytywanych w trakcie gry, umożliwiająca zabawę offline we wcześniej uruchamiane tematy z automatyczną aktualizacją pytań po połączeniu z siecią.
